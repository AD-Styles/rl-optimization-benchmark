...
본 프로젝트는 강화학습 에이전트가 단일 환경의 튜토리얼 수준을 넘어, 점진적으로 고도화되는 3단계 도장깨기(Taxi -> CliffWalking -> Blackjack)를 하나의 알고리즘으로 정복한 포트폴리오입니다. 단순 코드 재사용부터 보상 설계, 그리고 자료구조의 확장까지 엔지니어링 역량이 발전하는 과정을 데이터로 증명합니다.

## 🎯 핵심 목표 (Motivation)
| 핵심 역량 | 적용 환경 | 상세 목표 및 엔지니어링 포인트 |
| :--- | :---: | :--- |
| **Code Reusability** | **Taxi** | 동일한 Q-Learning 알고리즘으로 완전히 다른 환경을 풀어내는 알고리즘의 범용성 확인. |
| **Domain-Specific<br>Reward Shaping** | **CliffWalking** | 가혹한 패널티가 학습에 미치는 영향을 분석하고, 보상 재설계를 통해 최적 정책 도출. |
| **Universal Hash-based<br>Q-Table** | **Blackjack** | 정수형 상태를 넘어 튜플(Tuple) 형태의 다차원 상태 공간을 수용하는 해시 기반 에이전트 설계. |
...
각 단계별 환경에서 동일한 모델에 대해 학습률(`Alpha=0.1` vs `Alpha=0.5`)을 다르게 적용하여 수렴 안정성을 교차 검증했습니다.
...
## 5. 💡 회고록 (Retrospective)
* **Stage 1 (Taxi) - 알고리즘의 위력 체감:** 기존 코드를 재사용하여 500개의 상태를 가진 택시 환경이 즉시 학습되는 것을 보았을 때의 놀라움은 잊을 수 정 없습니다. 도메인이 달라도 마르코프 결정 과정(MDP)이라는 본질만 같다면, 하나의 범용 알고리즘으로 통제할 수 있다는 강화학습의 강력함을 확인했습니다.

* **Stage 2 (CliffWalking) - 통제된 패널티가 만들어낸 지능:** 절벽 문제의 핵심은 '보상의 균형'이었습니다. 에이전트에게 너무 가혹한 패널티(-100)는 학습 자체를 마비시켰습니다. 수백 번의 파라미터 튜닝과 패널티 조정을 반복하며 수렴 곡선을 띄워 올렸을 때, 인공지능은 맹목적인 코딩의 산물이 아니라 엔지니어가 통제한 환경 생태계 위에서 피어나는 결과물임을 몸소 체득했습니다.

* **Stage 3 (Blackjack) - 데이터 구조의 한계를 넘다:** 마지막 관문인 Blackjack의 튜플 상태를 마주했을 때, 하드코딩으로 예외 처리를 하는 대신 `defaultdict` 기반의 범용 해시 Q-Table을 고안했습니다. 데이터의 형태에 구애받지 않는 유연한 아키텍처 설계가 실무에서 얼마나 강력한 확장성을 내는지 증명한 순간이었습니다.

위 3단계 도장깨기 프로젝트는 제 엔지니어링 사고방식을 단순한 스크립터에서 문제 해결형 아키텍트로 진화시켜 준 결정적 계기였습니다. 단일 튜토리얼을 따라 치는 수준을 넘어 알고리즘을 새로운 도메인에 이식하고, 환경의 병목을 찾아 보상을 재설계하며, 자료구조를 확장하여 시스템을 최적화하는 전 과정을 내재화했습니다. 어떤 조건에서도 가장 통찰력 있는 분석과 최적화된 경로(Optimal Path)를 찾아내는 가치를 증명하도록 마인드셋을 하게되는 실습이였습니다.
